...
%if 0%{?fedora} || 0%{?rhel} >= 8
BuildRequires: python%{pyver}-enum34
...
do
mv oslo-config-$i oslo-config-$i-%{python%{pyver}_version}
ln -s oslo-config-$i-%{python%{pyver}_version} oslo-config-$i-%{pyver}
ln -s oslo-config-$i-%{python%{pyver}_version} oslo-config-$i
done
popd
